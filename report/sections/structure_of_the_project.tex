...
\begin{itemize}
    ...
    \item \textbf{Nodes:} The core components of the system. Nodes
        ...
        fault tolerance. Each node plays three roles simultaneously:
        a \emph{coordinator} that handles client requests, a
        \emph{replica} that stores data and responds to quorum
        messages, and a \emph{network participant} that manages
        ring topology changes. These roles are implemented as
        separate logic classes (\texttt{CoordinatorLogic},
        \texttt{ReplicaLogic}, \texttt{NetworkLogic}) that share a
        single \texttt{NodeContext} state object.
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Manager Entity
        \node[manager] (Manager) at (0, 4.8) {Manager\\(System Admin)}; 

        % Client Entities
        \node[client] (C1) at (-6, 2) {Client 1}; 
        \node[client] (C2) at (-6, -1.5) {Client 2}; 

        % Manager to Node interactions (dashed red arrows)
        \draw[dashed_arrow] (Manager) -- node[right, align=left, xshift=1.5cm, pos=1.0] {join, leave,\\crash, recover} (N10);
        \draw[dashed_arrow] (Manager) to[bend left=15] (N20);

        % Manager to Client interactions
        \draw[dashed_arrow] (Manager) to[bend right=25] node[left, xshift=-0.2cm] {spawn} (C1);
    ...
    \end{tikzpicture}
    \caption{High-level architecture of the Mars Storage System.}
\end{figure}
